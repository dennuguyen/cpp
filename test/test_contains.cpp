...
TEST(begins_with_contiguous_subset, true) {
    ...
    EXPECT_TRUE(xtl::contains(alpha, test));
}
...
TEST(duplicated_value_in_alphabet, true) {
    ...
    std::string word = "bb";
    EXPECT_TRUE(xtl::contains(alpha, word));
}
...
TEST(noncontiguous_subset, false) {
    ...
    EXPECT_FALSE(xtl::contains(alpha, test));
}
...
TEST(single_char_alphabet, false) {
    ...
    EXPECT_FALSE(xtl::contains(alpha, word));
}
...
TEST(sentence1, sentences) {
    ...
    EXPECT_TRUE(xtl::contains(alpha, word1));
    EXPECT_TRUE(xtl::contains(alpha, word2));
    EXPECT_TRUE(xtl::contains(alpha, word3));
    EXPECT_FALSE(xtl::contains(alpha, word4));
}
